...
Во время разработки и интеграции компонентов возникали следующие ошибки и проблемы:

1. **Ошибка `405 Method Not Allowed` при использовании SSE на клиенте**
   - **Симптом:** `GET /api/analyze/stream?url=... 405 (Method Not Allowed)`.
   - **Причина:** Браузерный объект `EventSource` (используемый во фронтенде Fact Guard) способен отправлять только `GET` запросы. Ранее бэкенд на Go принимал для старта стриминга исключительно `POST`.
   - **Решение:** Обработчик `/api/analyze/stream` в бэкенде был обновлен для поддержки как `POST` (для расширения/бота), так и `GET` (параметры извлекаются из query-строки: `?url=...&text=...`).

2. **Зависание или отсутствие потока данных от SSE через NGINX (Буферизация)**
   - **Симптом:** События в поток не приходят по одному, а вываливаются разом в конце, или запрос обрывается по таймауту.
   - **Причина:** По умолчанию в Nginx включена буферизация. Он ждет заполнения буфера (или завершения ответа) перед отправкой данных клиенту.
   - **Решение:** В конфигурации Nginx (`nginx.conf`) для SSE роутов (или глобально для API) отключена буферизация `proxy_buffering off;`, выставлены таймауты `proxy_read_timeout` на час, и отправка заголовков сервером `Cache-Control: no-cache` и `Connection: keep-alive`.

3. **Блокировка API CORS-политикой при запросах с других доменов/портов**
   - **Симптом:** Ошибка `CORS policy: No 'Access-Control-Allow-Origin' header is present`.
   - **Причина:** Фронтенд (запущенный, например, на `localhost:80` или `localhost:8000`) пытался обратиться к API на порту `8080`.
   - **Решение:** Во все публичные (клиентские) ручки бэкенда (`Analyze`, `AnalyzeStream`, `Chat`) добавлены CORS заголовки: `w.Header().Set("Access-Control-Allow-Origin", "*")` и перехват HTTP-метода `OPTIONS` (preflight-запросы).
