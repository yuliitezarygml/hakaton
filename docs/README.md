...
│   ├── chain.go                  # Цепочка источников: BuildChain, ChainNode, Distortion
...
│
├── handlers/
│   ├── analyzer.go               # HTTP обработчики (/stream, /analyze, /health, /limits)
│   └── chain.go                  # POST /api/chain/stream — SSE цепочка источников
│
...
### `POST /api/chain/stream` — SSE цепочка источников

```json
{ "url": "https://example.com/article" }
```

Стримит события по SSE по мере анализа каждого узла цепочки:

| Событие          | Данные                                      |
|------------------|---------------------------------------------|
| `chain_start`    | `{ message: "..." }` — инициализация        |
| `chain_progress` | `{ message: "..." }` — текущий шаг          |
| `chain_node`     | `{ node: ChainNode }` — найденный узел      |
| `chain_done`     | `{ message: "Готово" }` — завершение        |
| `chain_error`    | `{ message: "..." }` — ошибка              |
...
```json
{
  ...
  "distortions": [
    {
      "type": "exaggeration",
      "original": "незначительный рост",
      "changed": "катастрофический обвал"
    }
  ],
...
Типы искажений: `exaggeration` (преувеличение) · `omission` (умолчание) · `addition` (добавление) · `change` (изменение)
...
```
POST /api/chain/stream { url: "https://..." }
        │
        ▼
  ШАГ 1 — Загрузка оригинала
    → ContentFetcher скачивает статью по URL
    → AI извлекает: тема + поисковый запрос + ключевые утверждения
    → Стримим chain_node (оригинал, is_original: true)
        │
        ▼
  ШАГ 2 — Поиск похожих публикаций
    → Serper API ищет похожие статьи (RU + EN)
    → До 5 уникальных доменов
        │
        ▼
  ШАГ 3 — Анализ каждой статьи
    → Загрузка → AI сравнивает с оригинальными утверждениями
    → Извлекает: искажения, distortion_score, summary
    → Стримим chain_node по мере анализа
        │
        ▼
  chain_done → все узлы в Extension popup показаны
```

**Требования:** нужен `SERPER_API_KEY` для поиска похожих статей.

**В расширении:** кнопка «🔗 Цепочка источников» в idle-виде открывает view-chain с карточками узлов, цветными бейджами искажений и ссылками на источники.
